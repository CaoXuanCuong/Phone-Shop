...
namespace BTL_Web
{
    public partial class Chitietsanpham : System.Web.UI.Page
    {
        ...
        {
            query = "select * from tbl_DanhGia inner join tbl_User on tbl_DanhGia.sUserName = tbl_User.sUserName where sID_SanPham = @id and sID_DanhGia like '%DG%' order by dThoigian DESC";
            id = Convert.ToString(Request.QueryString["sID_SanPham"]);
            SqlCommand cmd = new SqlCommand(query, data.conn);
        ...
        protected void lvSPLienQuan_ItemCommand(object sender, ListViewCommandEventArgs e)
        {
            if ((bool)Session["login"] == true)
            {
                ...
                add.addItemCart((Label)e.Item.FindControl("lblIDSP"), (Label)e.Item.FindControl("lblGiaMoi"), 1, Session["Username"].ToString(), this.Page);
            }
            else
            {
        ...
        protected void FormView1_ItemCommand(object sender, FormViewCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Them")
            {
                ...
                {
                    AddToCart add = new AddToCart();
                    add.addItemCart(FormView1.FindControl("txtIDSP") as Label, FormView1.FindControl("txtGia") as Label, int.Parse(txtSL.Text), Session["Username"].ToString(), this.Page);
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this, typeof(Page), "alert", "alert('Bạn cần đăng nhập để sử dụng chức năng này.');", true);
